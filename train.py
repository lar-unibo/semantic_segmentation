...
hyperparameter_defaults = dict(
    backbone = "convnextT", # resnet50, resnet101, swinT, swinS, swinB, swinL, convnextT, convnextS, convnextB, convnextL, convnextXL
    epochs = 50,
    lr = 1e-5,
    batchsize = 2,
...
wandb.init(config=hyperparameter_defaults, project="fcd_segmentation_bags", mode="disabled")
config = wandb.config
...
transforms_base = aug.Compose([
    ...
    aug.RandomSizedCrop(min_max_height=[config["img_height"]-10, config["img_height"]], height=config["img_height"], width=config["img_width"], p=0.5),
...
def train_net(net,
    ...
    # Dataset ----------------------------------------------------------------------------------
    train_dataset = BasicDataset(train_dir, config["img_height"], config["img_width"], transform=eval(config["transforms"]))
    val_dataset = BasicDataset(val_dir, config["img_height"], config["img_width"], transform=eval(config["transforms"]))
    n_val = len(val_dataset)
    n_train = len(train_dataset)

    source_train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=config["batchsize"], shuffle=True, num_workers=8, pin_memory=True, drop_last=True)
    source_val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=config["batchsize"], shuffle=False, num_workers=8, pin_memory=True, drop_last=True)
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{config["epochs"]}', unit='img') as pbar:
            for img, mask in source_train_loader:
                ...
                if config["warmup_steps"] > 0 and global_step <= config["warmup_steps"]:
                    lr = config["warmup_lr"] + (config["lr"] - config["warmup_lr"]) * float(global_step / config["warmup_steps"])
                ...
                if global_step % (n_train // (config["freq_validation_per_epoch"] * config["batchsize"])) == 0:
                    ...
                    val_score, val_loss = eval_net(net=net, loader=source_val_loader, device=device, global_step=global_step)
                    wandb.log({'val_loss': val_loss, 'val_score': val_score, 'lr': optimizer.param_groups[0]['lr']}, step=global_step)
...
if __name__ == '__main__':
    ...
    model = MODEL_MAP[config["backbone"]](num_classes=1, output_stride=16, pretrained_backbone=config["pretrained_backbone"])
